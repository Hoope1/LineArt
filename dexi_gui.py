    ...
    try:
        subprocess.run(
            ["vtracer", "--input", str(png_path), "--output", str(svg_path),
             "--mode", "polygon", "--filter-speckle", "8", "--hierarchical", "true"],
...
def sd_refine(base_rgb, ctrl_L, steps=32, guidance=6.0, ctrl_scale=1.0, strength=0.70, seed=42, max_long=896):
    import torch
    ...
    img = pipe(
        prompt=("clean black-and-white line art, uniform outlines, detailed, "
...
def process_one(path: Path, out_dir: Path, cfg, log):
    ...
    if cfg["use_sd"]:
        refined, bw = sd_refine(
            ...
            ctrl_scale=cfg["ctrl"], strength=cfg["strength"],
            seed=cfg["seed"], max_long=cfg["max_long"]
        )
...
def process_folder(inp_dir: Path, out_dir: Path, cfg, log, done_cb):
    ...
    if not imgs:
        log("Keine Eingabebilder gefunden."); done_cb(); return
    for p in imgs:
...
class App(tk.Tk):
    ...
    def _build(self):
        ...
        ttk.Checkbutton(frm_opts, text="SD-Refinement (SD1.5 + ControlNet Lineart)", variable=self.use_sd).grid(row=0, column=0, sticky="w", columnspan=3)
        ttk.Checkbutton(frm_opts, text="SVG speichern (VTracer)", variable=self.save_svg).grid(row=0, column=3, sticky="w")

        ttk.Label(frm_opts, text="Steps").grid(row=1, column=0, sticky="e"); ttk.Entry(frm_opts, textvariable=self.steps, width=6).grid(row=1, column=1, sticky="w")
        ttk.Label(frm_opts, text="Guidance").grid(row=1, column=2, sticky="e"); ttk.Entry(frm_opts, textvariable=self.guidance, width=6).grid(row=1, column=3, sticky="w")
        ttk.Label(frm_opts, text="Ctrl-Scale").grid(row=1, column=4, sticky="e"); ttk.Entry(frm_opts, textvariable=self.ctrl, width=6).grid(row=1, column=5, sticky="w")

        ttk.Label(frm_opts, text="Strength (img2img)").grid(row=2, column=0, sticky="e"); ttk.Entry(frm_opts, textvariable=self.strength, width=6).grid(row=2, column=1, sticky="w")
        ttk.Label(frm_opts, text="Seed").grid(row=2, column=2, sticky="e"); ttk.Entry(frm_opts, textvariable=self.seed, width=8).grid(row=2, column=3, sticky="w")
        ttk.Label(frm_opts, text="Max lange Kante (px)").grid(row=2, column=4, sticky="e"); ttk.Entry(frm_opts, textvariable=self.max_long, width=6).grid(row=2, column=5, sticky="w")

        # --- Presets ---
        frm_presets = ttk.LabelFrame(self, text="Presets")
        frm_presets.pack(fill="x", **pad)

        ttk.Button(frm_presets, text="Technische Strichzeichnung",
                   command=self.preset_technical).grid(row=0, column=0, padx=4, pady=4, sticky="w")
        ttk.Button(frm_presets, text="Natürliche Lineart",
                   command=self.preset_natural).grid(row=0, column=1, padx=4, pady=4, sticky="w")

        frm_actions = ttk.Frame(self)
        frm_actions.pack(fill="x", **pad)

        ttk.Button(frm_actions, text="Modelle jetzt herunterladen", command=self.prefetch).pack(side="left")
        ttk.Button(frm_actions, text="Start", command=self.start).pack(side="left", padx=10)
        ttk.Button(frm_actions, text="Stopp", command=self.stop).pack(side="left")
    ...
    def preset_technical(self):
        """Technische Strichzeichnung: maximal einheitliche Konturen, klare Kanten, ideal für CAD-/Arbeitsblätter."""
    ...
    def preset_natural(self):
        """Natürliche Lineart: Hintergrund subtil erhalten, etwas weichere Linien bei hoher Detailtreue."""
        self.use_sd.set(True)
        self.save_svg.set(True)
        self.steps.set(32)
    ...
    def start(self):
        ...
        if not inp.exists():
            messagebox.showwarning("Fehler", "Bitte Eingabeordner wählen."); return
        if not out.exists():
